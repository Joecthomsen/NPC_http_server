...
router.post('/new_controle_gear', async (req, res, next) => {
    try {
        // Extracting required property
        const { manufactoringID, email } = req.body;
        if(!manufactoringID || !email) {
            return res.status(400).json({ error: 'Manufacturing ID and Email is required' });
        }
        ...
        const new_control_gear = await ControleGear.create({manufactoringID, email});
        if(!new_control_gear) {
...
router.post('/new_data_instance', async (req, res, next) => {
    try {
        // Extracting required property
